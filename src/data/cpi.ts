/**
 * US CPI-U annual averages (all items, 1982-84 = 100), from the Bureau of
 * Labor Statistics' published series. Used to translate historical dollar
 * amounts into today's money. 2024 is the latest complete year here —
 * conversions are labeled "in 2024 dollars".
 */

export const CPI_BASE_YEAR = 2024;

export const CPI_BY_YEAR: Record<number, number> = {
  ...
  2021: 271.0, 2022: 292.7, 2023: 304.7, 2024: 313.7,
};

/** Multiply an amount from `year` into CPI_BASE_YEAR dollars. */
export function inflationMultiplier(year: number): number | null {
  const from = CPI_BY_YEAR[year];
  const to = CPI_BY_YEAR[CPI_BASE_YEAR];
  if (!from || !to) {
    return null;
  }
  return to / from;
